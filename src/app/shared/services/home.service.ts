...
export class HomeService {
  ...
  openWsConnection() {
    ...
    this.socket.on('CANCELLED_ORDER_INQUIRY', (data) => {
      clearInterval(this.cancelledOrderNotificationSoundInterval);
      this.playNotificationSoundForNewInquireOrderIds[data['inquiry_order_data']['inquiry_order_id']] = false;
      this.showCancelledInquiryOrderDialog(data['inquiry_order_data']['inquiry_order_id']);
      this.setCancelledOrderNotificationSoundInterval();
      this.emitSocketEvent('CANCELLED_ORDER_INQUIRY', data);
    });
  }
  ...
  showCancelledInquiryOrderDialog(inquiryOrderId: number) {
    this.playNotificationSoundForCancelledOrderIds[inquiryOrderId] = true;
    this.dialogBoxForRecievingConsent(
      `Inquiry Order ID: ${inquiryOrderId} is cancelled`,
      inquiryOrderId
    );
  }
